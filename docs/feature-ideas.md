# 功能设计备忘

本文记录已经完成方案分析，但当前阶段不计划实现的功能设想。此类功能应具备明确的业务场景、职责边界和可行实现路径，以便后续根据项目目标选择性开发。

## 记录规则

- 只记录已经形成基本设计结论的功能，不记录尚未收敛的随想。
- 每个条目说明业务目标、建议流程、数据职责、当前不实现的原因和重新启动条件。
- 本文不作为当前开发计划；确定进入开发后，应在 `development-history.md` 末尾追加新阶段。
- 功能进入正式开发后，本文保留原设计结论，并补充对应开发阶段，不复制实现细节。
- 接口契约、当前调用链和实际实现状态仍分别以现有权威文档为准。

## 状态说明

| 状态 | 含义 |
| --- | --- |
| 备选 | 设计方向基本成立，但暂未确定是否实现 |
| 暂缓 | 当前不实现，等待明确业务价值或前置能力 |
| 已排期 | 已决定实现，应同步追加开发阶段 |
| 已实现 | 功能已经完成，具体实现以开发迭代记录为准 |
| 放弃 | 经过评估后不再考虑 |

## 转人工工单与 Agent 流程解耦

状态：暂缓

### 业务目标

当用户多次明确要求转人工，或当前问题无法由 Agent 有效解决时，允许 Agent 创建人工处理工单，将后续处理交给独立的后台业务流程。

### 建议流程

```mermaid
flowchart LR
    User[用户请求转人工] --> Agent[Agent 判断是否创建工单]
    Agent --> Tool[调用创建工单 Tool]
    Tool --> Backend[Java 业务后端]
    Backend --> Ticket[(工单数据)]
    Ticket --> Admin[后台人员处理]
    Admin --> Result[接受、拒绝或完成]
    Result --> Notify[邮件或会话通知]
    Notify --> User
```

1. Agent 根据当前会话判断是否需要转人工。
2. Agent 调用创建工单 Tool。
3. Java 后端创建 `INIT` 或 `PENDING` 状态的工单。
4. 本轮 LangGraph 正常结束，不等待后台人员处理。
5. 后台人员通过独立管理页面接受、拒绝或完成工单。
6. 业务后端更新工单状态，并通过邮件或会话通知告知用户。
7. 用户后续可以通过查询工单 Tool 获取最新处理结果。

### 设计结论

该流程不需要暂停原 LangGraph，也不需要使用 `interrupt` 或 `Command(resume=...)` 恢复原执行过程。

原因如下：

- 工单创建完成后，Agent 本轮任务已经结束。
- 后台处理可能发生在几分钟或几小时后，不应长期占用原 Graph 执行流程。
- 工单状态属于正式业务事实，应由业务数据库管理。
- 后台人员处理工单属于独立异步业务流程，不依赖原 Graph 的节点位置。
- 后续通知可以作为新的会话事件或通过查询 Tool 获取，不需要修改既有历史消息。

因此，该功能属于 Agent 与人工业务系统的异步协作，而不是本项目计划实现的 LangGraph Human-in-the-loop 示例。

### 数据职责

| 数据 | 建议存储位置 |
| --- | --- |
| 工单编号、状态、创建人和处理结果 | Java 业务数据库 |
| 工单创建动作 | Agent Tool 调用记录 |
| 后台处理记录 | 工单操作日志或审计表 |
| 邮件发送结果 | 通知记录 |
| 用户后续收到的状态变化 | 新的会话通知或查询结果 |
| 当前会话内的转人工请求次数 | LangGraph State，可由 Checkpointer 持久化 |

### 可选实现

后续实现时，可以增加：

- 创建工单 Tool
- 查询工单 Tool
- 工单状态机
- 后台待处理列表
- 接受、拒绝和完成接口
- 邮件通知
- 会话事件通知
- 重复创建防护与幂等控制
- 工单处理超时与 SLA

### 当前不实现的原因

- 核心流程简单，现阶段不具备足够的学习增量。
- 当前项目优先补齐 LangGraph State、Checkpointer、短期记忆和 Human-in-the-loop。
- 实现后台页面、通知和工单状态机会扩大业务开发范围，但对当前 Agent 能力展示提升有限。
- 该功能设计已经明确，后续可在需要时快速交由开发工具实现。

### 重新启动条件

满足以下任一条件时，可以将该功能加入开发计划：

- 需要展示 Agent 与传统业务后台的异步协作。
- 岗位要求包含客服工单、人工接管或任务流转。
- 项目已经完成 Checkpointer 和 Human-in-the-loop，仍需要增加一个独立业务闭环。
- 需要实现实际可用的人工客服后台。


## 生产级高风险操作的身份、授权与恢复边界

状态：暂缓

### 业务目标

当 Agent 在多用户环境中执行删除、修改、审批等高风险操作时，确保发起、确认和执行阶段均绑定可信用户身份，并在恢复执行前重新校验当前权限和业务事实，防止仅凭客户端提供的 thread_id 或 Checkpoint 中的历史权限完成写操作。

### 当前 Demo 已知限制

当前删除确认 Demo 已实现：

- LangGraph interrupt 暂停执行。
- Checkpointer 保存待确认状态。
- SSE 返回结构化人工动作事件。
- 使用同一 thread_id 恢复执行。
- execute 阶段重新校验业务对象。

但以下内容暂不实现：

- 真实 SSO/JWT 身份注入。
- resume 阶段操作者身份校验。
- 权限系统实时查询。
- 审批动作持久化。
- 幂等执行与审计。

### 设计结论

生产环境中应遵循：

- thread_id 只用于定位 Graph 执行实例，不作为身份凭证。
- Checkpoint 中保存的 user_context 只能用于恢复上下文，不能作为最终权限来源。
- prepare 阶段产生的是待确认动作，不代表最终授权。
- resume 时必须重新确认当前用户身份，并验证其是否有权执行该动作。
- execute 阶段必须重新读取业务事实和当前权限。
- 确认动作应该绑定 action_id、目标对象、发起人、确认人和有效期，而不是简单 boolean approval。
- 权限、审计、幂等属于业务系统职责，不属于 LangGraph State 职责。

### 建议后续实现

- SSO/OIDC/JWT 集成。
- AuthenticatedUser 上下文注入。
- PermissionProvider 权限查询抽象。
- ApprovalRecord 审批记录。
- action_id 生命周期管理。
- 确认超时与撤销。
- 幂等键和重复恢复保护。
- 审计日志。
- 拒绝、过期、重复确认测试。

### 当前不实现原因

- 当前阶段目标是验证 LangGraph HITL 机制，而不是实现完整企业权限平台。
- Demo 已经展示 interrupt、checkpoint、SSE 和恢复执行核心链路。
- 引入 SSO、IAM、审批数据库和审计系统会扩大工程范围，但不会增加当前学习目标。
- 当前删除操作只使用内存数据，不涉及真实业务风险。

### 重新启动条件

满足以下条件时考虑实现：

- Agent 开始操作真实业务数据。
- 接入企业账号体系。
- 支持多个用户同时使用。
- 需要满足审计或合规要求。
- Tool 扩展到删除、修改、审批、付款等高风险场景。